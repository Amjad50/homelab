{ ... }:
{
  ...
  networking.firewall.extraCommands = ''
    ...
    iptables -A INPUT -p tcp -m multiport --dport 8080,19999 -s 172.16.0.0/12 -j ACCEPT
    # Allow from localhost
    iptables -A INPUT -p tcp -m multiport --dport 8080,19999 -s 127.0.0.1 -j ACCEPT
    # Drop everything else
    iptables -A INPUT -p tcp -m multiport --dport 8080,19999 -j DROP
  '';

  networking.firewall.extraStopCommands = ''
    iptables -D INPUT -p tcp -m multiport --dport 8080,19999 -s 172.16.0.0/12 -j ACCEPT 2>/dev/null || true
    iptables -D INPUT -p tcp -m multiport --dport 8080,19999 -s 127.0.0.1 -j ACCEPT 2>/dev/null || true
    iptables -D INPUT -p tcp -m multiport --dport 8080,19999 -j DROP 2>/dev/null || true
  '';
}
